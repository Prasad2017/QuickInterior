...
public class Expense extends AppCompatActivity {
    ...
    @BindViews({R.id.amount, R.id.description})
    ...
    public void onClick(View view){
        switch (view.getId()){
            case R.id.SaveButton:
                if(validate(editTexts.get(0)) && validate(editTexts.get(1))){
                    addExpense();
                }else {
            ...
            case R.id.back:
                Intent intent=new Intent(Expense.this, MainPage.class);
...
//                        quatationId = quatationResponseList.get(i).getSer_id();
    ...
    private void addExpense() {
        ...
        ApiInterface apiInterface = ApiClient.getClient().create(ApiInterface.class);
        Call<LoginResponce> call = apiInterface.addExpense(MainPage.userId, editTexts.get(0).getText().toString().trim(), editTexts.get(1).getText().toString().trim(), quatationName);
